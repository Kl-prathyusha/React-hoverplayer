...
export default function HoverPlayer() {
  ...
  const hoveredInfo = useHoveredParagraphCoordinate(readableElements);

  // On mount: parse top-level readable elements
  useEffect(() => {
    const parsed = getTopLevelReadableElementsOnPage();
    setReadableElements(parsed);
  }, []);

  if (!hoveredInfo) return null;

  const { top, left, element, heightOfFirstLine } = hoveredInfo;
  ...
  return (
    ...
    style={{
      position: "absolute",
      top: top,
      left: left + element.offsetWidth + 10,
      zIndex: 1000,
    }}
    onClick={() => speechify(element)}
  >
    <PlayButton />
  </div>
  );
